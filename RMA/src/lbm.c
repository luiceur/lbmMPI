...
*/
            for ( int i=0; i<6; i++ ) {
                ...
                if ( ! ghost[GY(y)][GX(x)] ) {
                    lattice[LIB(x, y)].density[i][NOW] = lattice[LIB(x, y)].density[i][NEXT] =
		      1.0/6.0;
                }
			  ...
			  lattice[LIB(x, y)].density[i][NEXT] = 1.0/6.0; // No
                    }

                    else {
                        lattice[LIB(x, y)].density[(i+3)%6][NOW] = 
			  lattice[LIB(x, y)].density[i][NEXT] = 0.0; // Yes
                    }
                }
            }
        }
...
void init_datatypes() {
    ...
    MPI_Type_create_struct(3,(int[]){2,2,2*6},
                           (MPI_Aint[]){offsetof(point_t,position), 
			       offsetof(point_t, velocity), offsetof(point_t, density)},
...
void init_datastructures() {
  ...
  lattice = (point_t*)malloc(winsize);
  // displacements counted in units of integers
  disp_unit = sizeof(point_t);
  MPI_Win_create(lattice, winsize, disp_unit, MPI_INFO_NULL, MPI_COMM_WORLD, &win);
  
  memset(lattice, 0, winsize);
...
void rma_exchange(){
  ...
  MPI_Put(&lattice[LI(local_grid_width_halo-2, 1)], 1, border_col, east, 
	  LI(0, 1), 1, border_col, win);
  
  MPI_Put( &lattice[LI(1, 1)], 1, border_col, west, 
	   LI(local_grid_width_halo-1, 1), 1, border_col, win);
  
  MPI_Barrier(MPI_COMM_WORLD);
  
  MPI_Win_fence(0, win);
  
  MPI_Put(&lattice[LI(0, 1)], 1, border_row, north, 
	  LI(0, local_grid_height_halo-1), 1, border_row, win);
  
  MPI_Put(&lattice[LI(0, local_grid_height_halo-2)], 1, border_row, south, 
	  LI(0, 0), 1, border_row, win);
   MPI_Barrier(MPI_COMM_WORLD);
  MPI_Win_fence(0, win);
...
/*
    ...
    MPI_CHECK(MPI_Isend(&lattice[LI(local_grid_width_halo-2, 1)], 1, border_col, east, 0, 
    cartesian, &requestEast));
    MPI_CHECK(MPI_Irecv(&lattice[LI(local_grid_width_halo-1, 1)], 1, border_col, east, 1, 
    cartesian, &requestEast));

    // WEST
    MPI_Request requestWest;
    MPI_CHECK(MPI_Isend(&lattice[LI(1, 1)], 1, border_col, west, 1, cartesian, 
    &requestWest));
    MPI_CHECK(MPI_Irecv(&lattice[LI(0, 1)], 1, border_col, west, 0, cartesian, 
    &requestWest));
...
/*
    ...
    MPI_CHECK(MPI_Isend(&lattice[LI(0, 1)], 1, border_row, north, 0, cartesian, 
    &requestNorth));
    MPI_CHECK(MPI_Irecv(&lattice[LI(0, 0)], 1, border_row, north, 0, cartesian, 
    &requestNorth));

    // SOUTH
    MPI_Request requestSouth;
    MPI_CHECK(MPI_Isend(&lattice[LI(0, local_grid_height_halo-2)], 1, border_row, south,
    0, cartesian, &requestSouth));
    MPI_CHECK(MPI_Irecv(&lattice[LI(0, local_grid_height_halo-1)], 1, border_row, south, 
    0, cartesian, &requestSouth));
