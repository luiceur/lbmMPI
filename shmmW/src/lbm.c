...
void print_status(int out_rank, int out_nprocs, int out_dims[2], int out_coords[2],
		  ...
		  int out_local_grid_width,
                  int out_local_grid_height, int out_local_grid_width_halo, 
		  int out_local_grid_height_halo);
      ...
      */
      for ( int i=0; i<6; i++ ) {
        ...
        if ( ! ghost[GY(y)][GX(x)] ) {
          ...
          if ( ! ghost[n_y][n_x] ) {
            lattice[LIB(x, y)].density[(i+3)%6][NOW] = lattice[LIB(x, y)].density[i][NEXT] =
	      1.0/6.0; // No
          }
