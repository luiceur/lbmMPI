...
main ( int argc, char **argv )
{
    ...
    mat2 = malloc ( L_WIDTH*L_HEIGHT*sizeof(float) );
    ...
    /* Laplacian */
    memset ( mat2, 0, L_WIDTH*L_HEIGHT*sizeof(float) );
    for ( int i=1; i<L_HEIGHT-1; i++ )
        for ( int j=1; j<L_WIDTH-1; j++ )
            MAT2(i,j) = 4*MAT(i,j) -
                MAT(i-1,j) - MAT(i+1,j) - MAT(i,j-1) - MAT(i,j+1);
    memcpy ( mat, mat2, L_WIDTH*L_HEIGHT*sizeof(float) );

    /* Normalize locally (by vertical displacement) */
